...
2. **Channel picker** — a real multi-select (chips + "add a channel"
   dropdown, `#hubstaff-alerts` selected by default), not a single hardcoded
   chip, matching the copy's own "choose one or more channels" and the
   multi-channel example in the Figma source. Paired with the real
   ...
   and everyone in the chosen channel(s) will see it, rather than defaulting
   to `#general`. The "Connect channels" button is disabled if every channel
   is removed.
